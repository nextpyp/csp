...
cd $PBS_O_WORKDIR
echo -e "Running on directory: \\${PBS_O_WORKDIR}"
...
do
	...
	rm -f ${experimentName}_CSP_${iformat}.log
        ...
        if ! [ -f frealign/frealign_parameters_`printf %02d $iteration` ]
        then
            echo ERROR - Cannot find frealign/frealign_parameters_`printf %02d $iteration`
            break
        fi
	...
	#	mode=2;
		...
		command="mpiexec -n $np ${CSPDIR}/bin/csp_single_stack ${parameters_file} $i $mode ${UseImagesForRefinementMin} ${UseImagesForRefinementMax}"
		...
		cp ${experimentName}_CSP_${jformat}.parx ${experimentName}_${jformat}.par 
...
# 	fi
	...
	do
		...
		com="frealign_iter.sh $iteration 0 $cutoff 1 1"
		...
		#rm -f $mpirunfile
                ...
                # re-incorporate extended information (FAST!)
                
                cat maps/${experimentName}_${prevf}.par | grep C > newfile
                cat maps/${experimentName}_${prevf}.par | grep -v C | cut -b104- > extended
                cat maps/${experimentName}_${iformat}.par | grep -v C > standard
                paste standard extended >> newfile
                rm -f standard extended
                
		# replace parameter files with most up-to-date
                mv newfile maps/${experimentName}_${prevf}.par
	...
	#mv dummy ${experimentName}_CSP_${iformat}.parx
        ...
        #cat ${experimentName}_${iformat}.parx | grep C > newfile
        #cat ${experimentName}_${iformat}.parx | grep -v C | cut -b104- > extended
        #cat ${experimentName}_${iformat}.par | grep -v C > standard
        #paste standard extended >> newfile
        #
        ## replace parameter files with most up-to-date
        #mv newfile ${experimentName}_CSP_${iformat}.parx

        cp ${experimentName}_${prevf}.par ${experimentName}_CSP_${iformat}.parx
