...
bool ExtractParticlesApp::DoCalculation()
{
	...
	for ( int counter = 0; counter < number_of_particles; counter ++ )
	{
		 ...
		 */
		my_x = plt_x; // (-1.0) * ((plt_y - 1.0) - micrograph.logical_x_dimension + micrograph.physical_address_of_box_center_x);
		my_y = plt_y; // (plt_x - 1.0) - micrograph.physical_address_of_box_center_y;
		wxPrintf( "%f %f\n", my_x, my_y );
